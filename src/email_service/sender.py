...
class EmailSender:
    ...
    def create_excel_attachment(self, data, filename=None):
        """
        ...
        """
        if not data:
            logger.warning("No data provided for Excel attachment")
            return None
        ...
        try:
            ...
            with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                df.to_excel(writer, sheet_name='WAIIS_Immunization_Data', index=False)
            
            excel_buffer.seek(0)
            logger.info(f"Excel attachment created with {len(data)} records")
            return excel_buffer
...
WAIIS Automation System
        ...
        if data:
            excel_buffer = self.create_excel_attachment(data)
            if excel_buffer:
                attachment = MIMEBase('application', 'vnd.openxmlformats-officedocument.spreadsheetml.sheet')
                attachment.set_payload(excel_buffer.read())
                encoders.encode_base64(attachment)
                
                filename = f"WAIIS_Immunization_Report_{school_year_str}_{datetime.now().strftime('%Y%m%d')}.xlsx"
